...
class GitDataCollector(DataCollector):
    def collect(self, dir):
        ...
        lines = getpipeoutput(['git rev-list --pretty=format:"%%at %%ai %%aN <%%aE>" %s' % getcommitrange('HEAD'),
                               'grep -v ^commit']).split('\n')
        for line in lines:
            parts = line.split(' ', 4)
            author = ''
        ...
        revlines = getpipeoutput(
            ['git rev-list --pretty=format:"%%at %%T" %s' % getcommitrange('HEAD'), 'grep -v ^commit']).strip().split(
            '\n')
        lines = []
        revs_to_read = []
        time_rev_count = []
        ...
        lines = getpipeoutput(
            ['git log --shortstat %s --pretty=format:"%%at %%aN" %s' % (extra, getcommitrange('HEAD'))]).split('\n')
        lines.reverse()
        files = 0;
        inserted = 0;
        deleted = 0;
        ...
        lines = getpipeoutput(
            ['git log --shortstat --date-order --pretty=format:"%%at %%aN" %s' % (getcommitrange('HEAD'))]).split('\n')
        lines.reverse()
        files = 0;
        inserted = 0;
        deleted = 0
...
class HTMLReportCreator(ReportCreator):
    ...
    def createGraphs(self, path):
        ...
        for a in self.authors_to_plot:
            i = i + 1
            plots.append(
                """'lines_of_code_by_author.dat' using 1:%d title "%s" w lines""" % (i, a.replace("\"", "\\\"")))
        f.write(", ".join(plots))
        f.write('\n')
